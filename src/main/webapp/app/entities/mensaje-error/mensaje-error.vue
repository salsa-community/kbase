<template>
  <div>
    <h2 id="page-heading">
      <div>
        <el-input id="buscador-id" placeholder="Buscar código" v-model.trim="codigoSearch" @keyup.enter.native="buscarPorCodigo">
          <el-button slot="append" icon="el-icon-search" :loading="isFetching" @click="buscarPorCodigo"></el-button>
        </el-input>
      </div>
      <br />
      <router-link
          ...
          <tr v-for="mensajeError in mensajeErrors" :key="mensajeError.id">
            ...
            <td class="text-left">
              <div class="btn-group">
                ...
                <b-button v-on:click="prepareRemove(mensajeError)" variant="outline-danger" class="btn btn-sm">
    ...
    <b-modal header-text-variant="light" header-bg-variant="primary" ref="removeEntity" id="removeEntity">
      <span slot="modal-title">
        <span id="kbaseApp.mensajeError.delete.question" v-text="$t('entity.delete.title')">Confirm delete operation</span>
      </span>
