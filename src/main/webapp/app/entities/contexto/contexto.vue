<template>
  <div>
    <h2 id="page-heading">
      ...
      <router-link
        id="create-contexto-id"
        :to="{ name: 'ContextoCreate' }"
        tag="button"
        class="btn btn-primary float-right jh-create-entity create-contexto"
      >
        <span v-text="$t('kbaseApp.contexto.home.createLabel')"> Create a new Contexto </span>
      </router-link>
    </h2>
    <b-alert
    ...
    <div class="table-responsive" v-if="contextos && contextos.length > 0">
      <table class="table">
        ...
        <tbody>
          <tr v-for="contexto in contextos" :key="contexto.id">
            ...
            <td class="text-left">
              <div class="btn-group">
                <router-link
                  v-if="false"
                  :to="{ name: 'ContextoView', params: { contextoId: contexto.id } }"
                  tag="button"
                ...
                <router-link
                  :to="{ name: 'ContextoEdit', params: { contextoId: contexto.id } }"
                  tag="button"
                  class="btn btn-outline-primary btn-sm edit"
                >
                  <span class="d-none d-md-inline" v-text="$t('entity.action.edit')">Edit</span>
                </router-link>
                <b-button v-on:click="prepareRemove(contexto)" variant="outline-danger" class="btn btn-sm" v-b-modal.removeEntity>
    ...
    <b-modal header-text-variant="light" header-bg-variant="primary" ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="kbaseApp.contexto.delete.question" v-text="$t('entity.delete.title')">Confirm delete operation</span></span
      >
      <div class="modal-body">
        <p id="jhi-delete-contexto-heading" v-bind:title="$t('kbaseApp.contexto.delete.question')">
          Are you sure you want to delete this Contexto?
        </p>
      </div>
      <div slot="modal-footer">
        ...
        <button
          ...
          id="jhi-confirm-delete-contexto"
          v-text="$t('entity.action.delete')"
          v-on:click="removeContexto()"
        >
          Delete
        </button>
      </div>
